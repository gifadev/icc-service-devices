...
import pancashiki.iodevices
import pancashiki.writers
import pancashiki.parsers
...
current_parser = None
logger = logging.getLogger('pancashiki')
__version__ = importlib.metadata.version('pancashiki')
...
class ListUSBAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        pancashiki.iodevices.USBIO().list_usb_devices()
        parser.exit()

def pancashiki_main():
    global current_parser

    # Don't crash due to encoding errors
    sys.stdout.reconfigure(errors="backslashreplace")
    
    # Load parser modules
    parser_dict = {}
    for parser_module in dir(pancashiki.parsers):
        if parser_module.startswith('__'):
            continue
        if type(getattr(pancashiki.parsers, parser_module)) == type:
            c = getattr(pancashiki.parsers, parser_module)()
    ...
    parser.add_argument('-V', '--version', action='version', version='PANCASHIKI {}'.format(__version__))
    ...
    io_device = None
    if args.serial:
        io_device = pancashiki.iodevices.SerialIO(args.serial, args.baudrate, not args.no_rts, not args.no_dsr)
    elif args.usb:
        io_device = pancashiki.iodevices.USBIO()
        if args.address:
        ...
        io_device = pancashiki.iodevices.FileIO(args.dump)
    else:
    ...
    if args.pcap_file == None:
        writer = pancashiki.writers.SocketWriter(GSMTAP_IP, GSMTAP_PORT, IP_OVER_UDP_PORT)
    else:
        writer = pancashiki.writers.PcapWriter(args.pcap_file, GSMTAP_PORT, IP_OVER_UDP_PORT)
    ...
    if args.serial or args.usb:
        ...
        if not (args.qmdl == None) and args.type == 'qc':
            current_parser.run_diag(pancashiki.writers.RawWriter(args.qmdl))
        if not (args.sdmraw == None) and args.type == 'sec':
            current_parser.run_diag(pancashiki.writers.RawWriter(args.sdmraw))
        else:
