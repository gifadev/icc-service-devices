...
import os, sys
import re
import pancashiki.util as util
import struct
import uuid
import zlib
import json

from pancashiki.parsers.qualcomm import diagcmd
from pancashiki.parsers.qualcomm.diaggsmlogparser import DiagGsmLogParser
from pancashiki.parsers.qualcomm.diagwcdmalogparser import DiagWcdmaLogParser
from pancashiki.parsers.qualcomm.diagumtslogparser import DiagUmtsLogParser
from pancashiki.parsers.qualcomm.diagltelogparser import DiagLteLogParser
from pancashiki.parsers.qualcomm.diag1xlogparser import Diag1xLogParser
from pancashiki.parsers.qualcomm.diagnrlogparser import DiagNrLogParser

from pancashiki.parsers.qualcomm.diagcommoneventparser import DiagCommonEventParser
from pancashiki.parsers.qualcomm.diaglteeventparser import DiagLteEventParser
from pancashiki.parsers.qualcomm.diaggsmeventparser import DiagGsmEventParser
from pancashiki.parsers.qualcomm.diagfallbackeventparser import DiagFallbackEventParser
...
if bitstring_ver >= version.parse('4.2.0'):
    ...
    raise Exception("PANCASHIKI requires bitstring>=3.1.7, recommends bitstring>=4.0.0")
...
class QualcommParser:
    def __init__(self):
        ...
        self.logger = logging.getLogger('pancashiki.qualcommparser')
    ...
    def prepare_diag(self):
        ...
        if diagcmd.DIAG_SUBSYS_ID_1X in self.log_id_range:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_1x(self.log_id_range[diagcmd.DIAG_SUBSYS_ID_1X], self.layers)), 0x1000, False)
        else:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_1x(layers=self.layers)), 0x1000, False)
        if diagcmd.DIAG_SUBSYS_ID_WCDMA in self.log_id_range:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_wcdma(self.log_id_range[diagcmd.DIAG_SUBSYS_ID_WCDMA], self.layers)), 0x1000, False)
        else:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_wcdma(layers=self.layers)), 0x1000, False)
        if diagcmd.DIAG_SUBSYS_ID_GSM in self.log_id_range:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_gsm(self.log_id_range[diagcmd.DIAG_SUBSYS_ID_GSM], self.layers)), 0x1000, False)
        else:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_gsm(layers=self.layers)), 0x1000, False)
        if diagcmd.DIAG_SUBSYS_ID_UMTS in self.log_id_range:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_umts(self.log_id_range[diagcmd.DIAG_SUBSYS_ID_UMTS], self.layers)), 0x1000, False)
        else:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_umts(layers=self.layers)), 0x1000, False)
        if diagcmd.DIAG_SUBSYS_ID_LTE in self.log_id_range:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_lte(self.log_id_range[diagcmd.DIAG_SUBSYS_ID_LTE], self.layers)), 0x1000, False)
        else:
            self.io_device.write_then_read_discard(util.generate_packet(diagcmd.log_mask_pancashiki_lte(layers=self.layers)), 0x1000, False)
    ...
    def postprocess_parse_result(self, parse_result):
        ...
        # Retrieve radio_id and timestamp, using default values if not provided
        radio_id = parse_result.get('radio_id', 0)
        ts = parse_result.get('ts', datetime.datetime.now())

        # Helper function to write CP or UP messages
        def write_messages(key, writer_method):
            if key in parse_result:
                # Process messages if no layer is specified or if the layer is valid
                if 'layer' not in parse_result or parse_result['layer'] in self.layers:
                    for sock_content in parse_result[key]:
                        writer_method(sock_content, radio_id, ts)

        # Write CP and UP messages if available
        write_messages('cp', self.writer.write_cp)
        write_messages('up', self.writer.write_up)

        # Process stdout data
        stdout_data = parse_result.get('stdout', "")
        if stdout_data:
            # Open connection to the SQLite database and create the table if it doesn't exist
            ...
            # Function to parse a log line and extract fields for the database
            def parse_log_line(l):
                if "GSM Serving Cell Burst Metric:" in l:
                    type_ = "GSM"
                    category = "Serving Cell"
                    sub_category = "Burst Metric"
                    m = re.search(r'ARFCN:\s*(\d+)', l)
                    arfcn = m.group(1) if m else ""
                    m2 = re.search(r'RxPwr:\s*([-.\d]+)', l)
                    rssi = m2.group(1) if m2 else ""
                    return (type_, category, sub_category, "", "", arfcn, rssi)
                elif "GSM Serving Cell New Burst Metric:" in l:
                    type_ = "GSM"
                    category = "Serving Cell"
                    sub_category = "New Burst Metric"
                    m = re.search(r'ARFCN:\s*(\d+)', l)
                    arfcn = m.group(1) if m else ""
                    m2 = re.search(r'RxPwr:\s*([-.\d]+)', l)
                    rssi = m2.group(1) if m2 else ""
                    ...
                    category = "SCell"
                    sub_category = ""
                    m = re.search(r'EARFCN:\s*(\d+)', l)
                    arfcn = m.group(1) if m else ""
                    m2 = re.search(r'Measured RSSI:\s*([-.\d]+)', l)
                    rssi = m2.group(1) if m2 else ""
                    return (type_, category, sub_category, "", "", arfcn, rssi)
                elif "5GMM" in l:
                    type_ = "5G"
                    category = ""
                    sub_category = ""
                    m = re.search(r'MCC/MNC:\s*(\d+)/(\d+)', l)
                    if m:
                        mcc = m.group(1)
                        mnc = m.group(2)
                    else:
                        mcc, mnc = "", ""
                    return (type_, category, sub_category, mcc, mnc, "", "")
                else:
                    return None

            # Set to track processed log lines to avoid duplicates
            processed_lines = set()

            # Process each log line from stdout
            for l in stdout_data.split('\n'):
                ...
                # Output or send the log line based on combine_stdout mode
                if self.combine_stdout:
                    ...
                    # Build unified output using parsed data (same as database entry)
                    parsed = parse_log_line(l)
                    if parsed:
                        type_, category, sub_category, mcc, mnc, arfcn, rssi = parsed
                        parts = [f"[{type_}]"]
                        if category:
                        ...
                        if arfcn:
                            # For LTE, use label EARFCN; for GSM, use ARFCN
                            if type_ == "LTE":
                                parts.append(f"EARFCN: {arfcn}")
                            else:
                                parts.append(f"ARFCN: {arfcn}")
                        if rssi:
                ...
                # Parse the log line and insert into the database if parsing is successful
                parsed = parse_log_line(l)
                if parsed:
                    cursor.execute("""
                        INSERT INTO log_table (type, category, sub_category, mcc, mnc, arfcn, rssi)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, parsed)
    ...
    #     def write_messages(key, writer_method):
    #         if key in parse_result:
    #             # Process messages if no layer is specified or the layer is valid
    #             if 'layer' not in parse_result or parse_result['layer'] in self.layers:
    #                 for sock_content in parse_result[key]:
    #                     writer_method(sock_content, radio_id, ts)
    ...
    #     if stdout_data:
    #         # Open connection to the SQLite database and create the table if it doesn't exist
    #         conn = sqlite3.connect("parse_result.db")
    #         cursor = conn.cursor()
    #         cursor.execute("""
    ...
    #             # Process the log line based on combine_stdout mode
    #             if self.combine_stdout:
    #                 osmocore_log_hdr = util.create_osmocore_logging_header(
    #                     timestamp=ts,
    #                     process_name=Path(sys.argv[0]).name,
    ...
    #             else:
    #                 if "GSM Serving Cell Burst Metric:" in l:
    #                     metric_data = l.replace("GSM Serving Cell Burst Metric:", "", 1).strip()
    #                     formatted = f"[GSM] » [Serving Cell] » {metric_data} » [Burst Metric]"
    #                     print(formatted)
    #                 elif "GSM Serving Cell New Burst Metric:" in l:
    #                     metric_data = l.replace("GSM Serving Cell New Burst Metric:", "", 1).strip()
    #                     formatted = f"[GSM] » [Serving Cell] » {metric_data} » [New Burst Metric]"
    #                     print(formatted)
    #                 elif "LTE SCell:" in l:
    #                     formatted = "[LTE] » [SCell] » " + l.replace("LTE SCell:", "", 1).strip()
    #                     print(formatted)
    #                 elif "5GMM" in l:
    #                     formatted = "[5GMM] » " + l.replace("5GMM", "", 1).strip()
    #                     print(formatted)
    ...
    #                 for l in parse_result['stdout'].split('\n'):
    #                     if "GSM Serving Cell Burst Metric:" in l:
    #                         print("[GSM] » [Serving Cell] » [Burst Metric] » " + l.replace("GSM Serving Cell Burst Metric: ", "", 1))
    #                     elif "GSM Serving Cell New Burst Metric:" in l:
    #                         print("[GSM] » [Serving Cell] » [New Burst Metric] » " + l.replace("GSM Serving Cell New Burst Metric: ", "", 1))
    #                     elif "LTE SCell:" in l:
    #                         print("[LTE] » [SCell] » " + l.replace("LTE SCell: ", "", 1))
    #                     elif "5GMM" in l:
    #                         print("[5GMM] » " + l.replace("5GMM ", "", 1))
